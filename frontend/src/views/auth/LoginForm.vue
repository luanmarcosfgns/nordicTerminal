<template>

  <div class="h-100 background-gradient">
    <div class="row">
      <div class="col-12">
        <div class="row d-flex justify-content-center align-items-center h-100">
          <div class=" col-sm-10 col-md-4 col-lg-4 col-12 pt-4">
            <div class="card">
              <div class="card-header pt-4 ps-4 pb-4">
                <div class="row">
                  <div class="col-12 ps-5">
                    <img class="justify-content-center align-items-center" width="200" src="@/assets/logo.png">
                  </div>

                </div>


              </div>
              <div class="card-body card-body-login">
                <div class="mb-4 mt-2 ">
                  <input type="text" class="form-control input-login" id="email" placeholder="Digite seu Usuário"
                  >
                </div>
                <div class="mb-4 ">
                  <input type="password" class="form-control input-login" placeholder="Digite seu Senha"
                         id="password">
                </div>
                <div class="row">
                  <div class="col-6">
                    <div class="mb-4 ">
                      <router-link aria-current="page" to="/esqueci-minha-senha">Esqueceu a senha?</router-link>
                    </div>
                  </div>
                  <div class="col-6">
                    <div class="float-end form-check">
                      <input class="form-check-input" type="checkbox" value="1" id="save">
                      <label class="form-check-label " for="flexCheckDefault">
                        Salvar a senha?
                      </label>
                    </div>
                  </div>
                </div>


                <div class="row">
                  <div class="col-12 d-grid gap-2 ps-4 pe-4 pt-4">
                    <button @click="login" class="btn btn-lg btn-system btn-loading">
                      Entrar
                    </button>
                  </div>
                  <div  v-if="VUE_APP_GOOGLE_LOGIN==='YES' && VUE_APP_AUTO_REGISTER==='YES'" class="col-12 d-grid gap-2 ps-4 pe-4 pt-4">
                    <button @click="register" class="btn btn-lg btn-ouro btn-loading">
                      Registrar-se
                    </button>
                  </div>
                  <div  v-if="VUE_APP_GOOGLE_LOGIN==='YES' && VUE_APP_AUTO_REGISTER==='YES'" class="col-12 d-grid gap-2 ps-4 pe-4 pt-4">
                    <button @click="loginWithGoogle" class="btn btn-lg btn-outline-dark btn-loading">
                      <svg height="30px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48">
                        <path fill="#EA4335"
                              d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"/>
                        <path fill="#4285F4"
                              d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"/>
                        <path fill="#FBBC05"
                              d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"/>
                        <path fill="#34A853"
                              d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/>
                        <path fill="none" d="M0 0h48v48H0z"/>
                      </svg>
                      Entrar com Google
                    </button>
                  </div>
                </div>
              </div>

            </div>

          </div>
        </div>

      </div>

    </div>

  </div>

</template>

<script>


import axios from "axios";
import toastr from "toastr/build/toastr.min";


import Helpers from "@/services/Helpers";
import Middleware from "@/services/Middleware";


export default {
  name: "LoginForm",
  computed: {},
  data() {
    return {
      appName: process.env.VUE_APP_APPLICATION_NAME,
      VUE_APP_GOOGLE_LOGIN: '',
      VUE_APP_AUTO_REGISTER: '',
    }
  }, methods: {
    login() {


      let email = document.getElementById('email').value;
      let password = document.getElementById('password').value;
      let save = document.getElementById('save').value;

      let data = {
        email: email,
        password: password,
        save: save
      };
      axios.post(process.env.VUE_APP_API_HOST_NAME + '/api/auth/login', data, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("HASH")}`,
          Accept: 'application/json',
          "Content-Type": 'application/json'
        }
      })
          .then(async function (response) {

            localStorage.setItem('HASH', response.data.data.token);


            let url = new Middleware().getRegisterLastRouteBeforeLogin()
            if (new Helpers().empty(url)) {
              url = '/painel'
            }

            location.href = url;

          }).catch(function (error) {


        if (error.request.status == 401) {

          toastr.error("Login ou senha invalidos");

          return false;
        }
        console.log(error)
        toastr.error(error.response.data.message);
      });
    },
    async loginWithGoogle() {
      try {
        const {data} = await axios.get(process.env.VUE_APP_API_HOST_NAME + '/api/auth/google/url');
        window.location.href = data.url;
      } catch (error) {
        console.error('Erro ao obter URL de autenticação:', error);
      }
    },
    register(){
      this.$router.replace('/register');
    }
  },
  mounted() {
    document.querySelector("body").classList.add('background-gradient');
    let helper = new Helpers();
    if (!helper.empty(localStorage.getItem("HASH"))) {
      location.href = '/connections/index';
    }
    this.VUE_APP_GOOGLE_LOGIN = process.env.VUE_APP_GOOGLE_LOGIN;
    this.VUE_APP_AUTO_REGISTER = process.env.VUE_APP_AUTO_REGISTER;

  }
}
</script>

<style scoped>


.input-login {
  height: 50px;
}

.card-body-login, .row, .content-wrapper {
  overflow: hidden !important;
}

.background-gradient {
  width: 100% !important;
  height: 100% !important;
  position: fixed !important;
  background: linear-gradient(0deg, rgba(34, 193, 195, 1) 0%, rgba(253, 187, 45, 1) 100%) no-repeat !important;

}


@import "toastr/build/toastr.css";

</style>